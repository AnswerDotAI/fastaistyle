...
IPython recognizes notebook commands such as `%time`, `v = %apl ⍳3`, `files = !ls`, and `name?`. The checker and autofixer preserve these commands and check the surrounding Python. Diagnostics use the original cell line numbers. Cell-magic bodies belong to their own language and are left intact.

Python modules whose header says `AUTOGENERATED! DO NOT EDIT!` (nbdev exports) are skipped by both checking and `--fix`. Check and fix the notebook that generates them.
