...
def _is_generated(source: str) -> bool:
    "nbdev writes this header into every exported module, after the module docstring when there is one. The notebook is the source to check."
    return any(line.startswith("# AUTOGENERATED! DO NOT EDIT!") for line in source.splitlines())
...
def _check_notebook_unused_imports(cells: list[dict], path: str, violations: list[tuple]):
    "Check unused imports across exported notebook cells; a skipped cell still counts as a user of imports but gets no issues of its own."
    export_cells = [cell for cell in cells if cell["export"]]
    ...
    suppressed = suppressed_lines(export_lines) | {n for n,(cell,_) in line_map.items() if cell["skip"]}
    non_export_cells = [cell for cell in cells if not cell["export"]]
    non_export_used = set()
